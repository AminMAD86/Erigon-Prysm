...
if $SUCCESS; then
  ...
  if command -v pv >/dev/null 2>&1 && [ -n "$SNAPSHOT_SIZE" ]; then
    ...
    wget -q -O - "$SNAPSHOT_URL" | pv -pterb -s "$SNAPSHOT_SIZE" | lz4 -d | tar -xf - || SUCCESS=false
  elif command -v pv >/dev/null 2>&1; then
    echo "• Could not determine snapshot size. Progress bar will be limited."
    wget -q -O - "$SNAPSHOT_URL" | pv | lz4 -d | tar -xf - || SUCCESS=false
  else
    echo "• pv not installed. No progress bar will be shown."
    wget -q -O - "$SNAPSHOT_URL" | lz4 -d | tar -xf - || SUCCESS=false
  fi
...
services:
  erigon:
    ...
    command:
      ...
      - --http.vhosts=*
      - --http.corsdomain=*
      - --http.api=eth,net,erigon,web3,debug,trace,txpool,engine
      - --ws
      - --ws.port=8546
      ...
      - --port=30303
      - --maxpeers=128
      - --p2p.protocol=68,67,69
      - --p2p.allowed-ports=30303,30304,30305,30306,30307,30308,30309,30310,30311,30312,30313,30314,30315
      - --nat=extip:$(curl -s ifconfig.me)
      - --nodiscover=false
      - --v5disc=true
      - --discovery.dns=all.sepolia.ethdisco.net,enr-tree://AKA3AM6LPBYEUDMVNU3BSVQJ5AD45Y7YPOHJLEF6W26QOE4VTUDPE@all.sepolia.ethdisco.net
      - --bootnodes=enode://4e5e92199ee224a01932a377160aa432f31d0b351f84ab413a8e0a42f4f36476f8fb1cbe914af0d9aef0d51665c214cf653c651c4bbd9d5550a934f241f1682b@138.197.51.181:30303
      - --batchSize=1GB
      ...
      - --torrent.conns.perfile=64
      - --torrent.maxpeers=128
      - --torrent.port=42069
      - --torrent.trackers.disable=false
      - --torrent.verbosity=2
      - --bodies.cache=3GB
      - --state.cache=2GB
      ...
      - --rpc.evmtimeout=15m0s
      - --rpc.overlay.getlogstimeout=15m0s
      - --etl.bufferSize=2GB
      - --persist.receipts=true
      - --database.verbosity=1
      - --private.api.ratelimit=31872
      - --state.stream.disable=false
